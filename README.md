...
**例外的に水色メイド(目隠れのメイドさん)、青色メイド、シアンメイド(オバケメイドさん)は瞬き部分のテスクチャを追加していないため、事実上常時NotSR2付きとなっています。**
青色メイドさんは目元のテスクチャが微妙に異なる関係で他のメイドさんと異なり、全く同じ方法を流用できないため、対応保留です。
...
ビルドしてから気がつきましたが、33.3%ではなく内部的には33.0%でした。 デバッグ時によく使っていた拡大率のため、+-5%区切りではないですが、同梱しています。
...
>   - 内部的にはクラス、テスクチャ全てサイズ毎に独立しているので、20モデル位の個別同時対応をするようなものではないかと個人的には考えています
...
<hr>
マルチモデルJavaソースコードライセンスとは、src/main/java/下にあるModelLittleMaid...から始まるJavaコードのことです。

リソースライセンスとはsrc/main/resources/下にあるpngファイル及び、配布zipに含まれるpngファイルのことです。
